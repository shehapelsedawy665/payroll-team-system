...
app.post("/api/calculate", async (req, res) => {
    ...
    let tax = 0;
    let temp = Math.max(0, annualTaxable - 40000);

    if (temp > 0) { let x = Math.min(temp, 15000); tax += x * 0.1; temp -= x; }
    if (temp > 0) { let x = Math.min(temp, 15000); tax += x * 0.15; temp -= x; }
    if (temp > 0) { let x = Math.min(temp, 130000); tax += x * 0.2; temp -= x; }

    const totalTax = R(tax / 360 * totalDays);
    let monthlyTax = R(totalTax - d.prevTaxes);
